...
const StartupDetailView: React.FC<StartupDetailViewProps> = ({
  ...
  const Kpi = ({ label, value }: { label: string; value: string }) => (
    <div className="bg-white dark:bg-[#0f141c] px-4 py-3">
      <div className="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">{label}</div>
      <div className="mt-0.5 font-semibold text-slate-900 dark:text-slate-100 tabular-nums">{value}</div>
    </div>
  );

  return (
    <div className="space-y-6 text-slate-900 dark:text-slate-100">
      <button onClick={() => setActiveView('startups')} className="flex items-center gap-1 text-sm text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className={`${card} p-6`}>
            <div className="flex items-center gap-4 mb-4">
              <div className="w-14 h-14 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-500 dark:text-slate-400 text-xl font-semibold overflow-hidden shrink-0">
                {data.logo ? <img src={data.logo} alt="" className="w-full h-full object-cover" /> : (data.name || '?').slice(0, 1)}
              </div>
              <div className="min-w-0">
                <h1 className="text-xl font-semibold text-slate-900 dark:text-slate-100">{data.name}</h1>
          ...
          <div className={card}>
            <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800"><h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Campagne</h2></div>
            <div className="p-6">
              <div className="flex justify-between text-sm mb-1 tabular-nums">
                <span className="text-slate-600 dark:text-slate-300">{fmtXOF(raised)} levés</span>
                <span className="text-slate-900 dark:text-slate-100 font-medium">{progress.toFixed(0)}%</span>
              </div>
              <div className="h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden mb-1"><div className="h-full bg-blue-600 rounded-full" style={{ width: `${progress}%` }} /></div>
              <div className="text-xs text-slate-400 dark:text-slate-500 mb-4">Objectif : {fmtXOF(target)}</div>
        ...
        <div className="lg:col-span-1">
          <div className={`${card} p-6 lg:sticky lg:top-24`}>
            <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100 mb-4">Investir</h2>
            {msg && (
              <div className={`mb-4 p-3 rounded-md text-sm ${msg.type === 'ok' ? 'bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 text-emerald-700 dark:text-emerald-400' : 'bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 text-red-700 dark:text-red-400'}`}>{msg.text}</div>
            )}
            ...
            <input
              ...
              className="w-full px-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-md text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:ring-2 focus:ring-blue-600 focus:border-transparent outline-none"
            />
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Min. {fmtXOF(min)}{max ? ` · Max. ${fmtXOF(max)}` : ''}</p>
            <button onClick={invest} disabled={submitting} className="mt-4 w-full bg-blue-700 dark:bg-blue-600 hover:bg-blue-800 text-white py-2.5 rounded-md font-medium transition-colors disabled:opacity-60">
